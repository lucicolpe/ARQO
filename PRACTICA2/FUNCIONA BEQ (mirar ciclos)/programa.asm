...
main:
  ...
  nop
  # RIESGOS DE CONTROL
  lw $t1, 12($zero) # en r9 guarda 8
  beq $t1, $zero, FINAL # no efectivo
  nop
  add $t2, $t1, $zero # en r10 guarda 8 
  beq $t2, $zero, FINAL # no efectivo
  nop
  lw $t2, 4($zero) # en r10 guarda 2
  nop
  beq $t2, $zero, FINAL # no efectivo
  nop
  add $t3, $t1, $t2 # en r11 guarda 3
  nop
  beq $t3, $zero, FINAL # no efectivo
  nop
  lw $t3, 8($zero) # en r11 guarda 4
  beq $t3, $t3, SALTO1 #efectivo
  nop
  nop
  add $t6, $t1, $t1 #No debería de ejecutarse
  nop
  nop
  SALTO1: add $t4, $t3, $t2 # en r12 guarda 6
  beq $t4, $t4, SALTO2 # efectivo
  nop
  nop
  add $t6, $t1, $t1 #No debería de ejecutarse
  nop
  nop
  SALTO2: lw $t4, 12($zero) # en r12 guarda 8
  nop
  beq $t4, $t4, SALTO3 #efectivo
  nop
  nop
  add $t6, $t1, $t1 #No debería de ejecutarse
  nop
  nop
  SALTO3: add $t5, $t3, $t4 #en r13 guarda 4+8=12
  nop
  beq $t5, $t5, SALTO4 #efectivo
  nop
  nop
  add $t6, $t1, $t1 #No debería de ejecutarse 
  FINAL: beq $zero, $zero, FINAL